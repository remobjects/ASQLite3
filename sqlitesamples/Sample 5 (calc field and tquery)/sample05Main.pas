...
type
  TForm1 = class(TForm)
    ...
    Label1: TLabel;
    ASQLite3Table1delta: TIntegerField;
    ASQLite3Query1: TASQLite3Query;
    procedure FormCreate(Sender: TObject);
    procedure RGDateClick(Sender: TObject);
    procedure ASQLite3Table1CalcFields(DataSet: TDataSet);
  private
...
procedure TForm1.ASQLite3Table1CalcFields(DataSet: TDataSet);
begin
  with ASQLite3Query1 do begin
    sql.text := 'select count(*) as c from children where parent=:p';
    params[0].AsInteger := DataSet.FieldByName('number').AsInteger;
    open;
    DataSet.FieldByName('delta').AsInteger := FieldByName('c').AsInteger;
  end;
end;
